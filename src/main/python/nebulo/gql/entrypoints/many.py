...
from nebulo.gql.entrypoints.resolver import async_resolver


def many_node_factory(sqla_model) -> Field:
    connection = connection_factory(sqla_model)
    return Field(connection, args=connection_args_factory(sqla_model), resolve=async_resolver, description="")
